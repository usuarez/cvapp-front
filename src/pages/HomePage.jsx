...
function HomePage() {
    ...
    return (
        <div className="app pb-5">
            {(!!auth._id) ? (!auth.address && !auth.dni && !auth.phone) ?
                <Redirect to='/new-profile' /> : null : null
            }
            ...
            <header className="homeHeader fluid-container d-flex justify-content-center align-items-center" style={{backgroundImage: `url('${background}')`}}>
                <div className="homeCopy col-12 col-sm-11 col-lg-8 d-flex flex-column justify-content-center align-items-center">
                    <h2 className="mt-0 mb-3 text-center thin">{(!!auth._id) ? `Hola, ${auth.name.split(' ')[0]}` : 'Plantillas de hojas de vida'}</h2>
                    ...
                    {(!!auth._id) ? null : <Link to="/login" className="text-center col-4 btn btn-next bg-complementary text-dark mt-3">Registrate</Link> }
            ...
            <main className="fluid-container pt-5">
                <section className="row pb-5 col-10 offset-1">
                    ...
                    <TemplateList />
                    {(!!auth._id) ? null : <Link to="/login" className="text-center col-4 offset-4 btn btn-next bg-shade type-fff text-dark mt-4">Registrate</Link>}
                    
                </section>
                <section className="row align-items-center mb-5 bg-white py-5 pb-5">
                    <div className="col-10 offset-1 mb-4 col-md-4 offset-md-2">
                        <h4 className="col-12">Tus datos en PDF</h4>
                        <p className="col-12 mb-3">Tu hoja de vida se guarda en formato portable y nadie tendrá problemas para poder ver el documento.</p>
                        
                        <Link to="/login" className=" col-6 offset-3 ml-15 offset-md-0 btn btn-next bg-complementary text-dark mt-4">Registrate</Link>
                    </div>
                ...
                <section className="row align-items-center py-5 pb-5">
                    ...
                    <div className="col-10 offset-1 mb-4 col-md-5 offset-md-0">
                        <h4 className="col-12">Procesamos tu foto de perfil con IA</h4>
                        <p className="col-12 mb-3">Nuestra inteligencia artificial seleccionará tu rostro y recortará tu foto de perfil. No era necesario pero ahi está.</p>
                        <Link to="/login" className=" col-6 offset-3 ml-15 offset-md-0 btn btn-next bg-complementary text-dark mt-4">Registrate</Link>
                    </div>
